...
class timeout {
  ...
  setTime(timerLength, timerLengthUnit) {
    if (isNaN(Number(timerLength)))
      throw new TypeError("timerLength must be numerical");
    if (this.#running) return false;
...
class interval {
  ...
  setTime(intervalLength, intervalLengthUnit) {
    if (isNaN(Number(intervalLength)))
      throw new TypeError("intervalLength must be numerical");
    if (this.#running) return false;

    this.#intervalLength = this.#convert(
      intervalLength,
      intervalLengthUnit || "ms"
    );
...
const printObject = (obj, level = 1, comma = false, maxdepth = false) => {
  ...
  const printKey = key =>
    isArray ? source.yellow(key) : source.blue(`"${key}"`);
  const inspection = inspect(obj);
  const isInstanceOf = !isArray
    ? source.cyan(inspection.substr(0, inspection.indexOf("{")))
    : "";
  ...
  for (let keyIndex in objKeys) {
    ...
    switch (typeof obj[key]) {
      case "function":
        print(
          `${spacing} ${printKey(key)}: ${source.cyan(`[Function: ${key}]`)}${
            isLastKey ? "" : ","
          }`
        );
        break;
...
const valueDescriptor = value => {
  if (value === null) {
    return source.grey("null");
  }
  if (value instanceof Promise) {
    return source.cyan(`${inspect(value)}`);
  }
  ...
  switch (typeof value) {
    case "function":
      return source.cyan(inspect(value));
    case "string":
      return source.blue(`"${value}"`);
    case "boolean":
      if (value === true) {
        return source.green("true");
      } else {
        return source.red("false");
      }
    case "undefined":
...
export { clamp, fetchJson, interval, log, range, timeout };
